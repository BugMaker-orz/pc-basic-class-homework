...
% 图片路径
\graphicspath{{./shots/q13/}{./shots/q14/}{./shots/q15/}{./shots/q16/}}
...
本次课后练习来自 \href{https://mlbo.github.io/missing-semester-cn-2026/2026/code-quality/}{Missing Semester 2026 第9讲：代码质量}，共完成 5 道题，涵盖 Makefile、版本约束、Git hooks、CI 等主题。

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{practice_4_1.png}
\caption{课后练习：代码质量（Makefile 与 .PHONY、版本约束、pre-commit hook、GitHub Actions）}
\label{fig:practice41}
\end{figure}

\begin{enumerate}
    \item \textbf{Makefile clean 目标}：在原有 \texttt{paper.pdf} 规则基础上加 \texttt{clean} 目标用 \texttt{rm -f paper.pdf} 删除产物，并在文件开头声明 \texttt{.PHONY: all clean}，防止当前目录下恰好有名为 \texttt{clean} 的文件时 make 跳过执行。
    \item \textbf{版本约束语法}：\texttt{\^{}} 表示兼容次版本（如 \texttt{\^{}1.2.3} 允许 1.9.0 但不允许 2.0.0）；\texttt{\textasciitilde} 表示只兼容补丁号（\texttt{\textasciitilde1.2.3} 允许 1.2.9 但不允许 1.3.0）；还有通配符 \texttt{*} 和比较符 \texttt{>=,<}。
    \item \textbf{pre-commit hook}：在 \texttt{.git/hooks/pre-commit} 写脚本，提交前自动跑 \texttt{make paper.pdf}，构建失败就 \texttt{exit 1} 拒绝提交。实测修改 \texttt{Makefile} 后 hook 正确拦截了不完整提交。
    \item \textbf{GitHub Pages + Action}：把静态页面推到 GitHub 仓库后启用 Pages 自动部署，同时配一个 Action 在每次 push 时对 \texttt{.sh} 文件跑 \texttt{shellcheck}，有警告就标红。
    \item \textbf{Markdown lint Action}：对 \texttt{.md} 文件跑 \texttt{proselint} 或 \texttt{write-good}，提交一篇有拼写错误的 md 文件验证 Action 能拦截。
\end{enumerate}
...
第四次实验共 4 题（第13、14、15、16题），已全部完成。第13题完成了 ruff + pytest 本地质量门禁；第14题完成了 Makefile 增量构建与 clean 验证；第15题完成了 HTTP 服务、curl/jq 数据处理与自动报告生成；第16题完成了陌生仓库的"破坏—失败—修复—构建—提交"综合检查。全部题目均配有终端截图与代码文件，代码与报告已按课程要求分次提交并推送到个人 GitHub 仓库。
...
按照课程要求，本次实验的所有代码和报告均通过 Git 进行版本管理，禁止单次全量提交。第四周（第13—16题）按功能模块分次提交，相关提交记录如下：

\begin{table}[H]
\centering
\caption{Git 提交记录（第四周）}
\label{tab:gitlog}
\begin{tabular}{lll}
\toprule
提交哈希 & 提交信息 & 说明 \\
\midrule
52c1d35 & docs(lab4): complete week4 report & 第13—16题报告（17页PDF） \\
eecfc2b & feat: q16 break-fix-build-commit & 第16题代码与README \\
98f5872 & feat: q15 api data to report & 第15题代码与README \\
4398053 & feat: q14 makefile incremental build & 第14题代码与README \\
01dd360 & feat: q13 quality gate & 第13题代码与README \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{git_log_week4.png}
\caption{Git 提交历史（第四周相关提交）}
\label{fig:gitlog}
\end{figure}
