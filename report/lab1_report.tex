...
\begin{itemize}
    ...
    \item 创建含空格文件名 \texttt{notes one.txt}、文件 \texttt{secret.txt}、空文件 \texttt{empty.txt} 和日志文件 \texttt{run.log}；
    \item 显示 \texttt{q01} 的绝对路径，以长格式列出 \texttt{input} 下全部项目；
...
\textbf{步骤2：显示绝对路径与文件列表。} 使用 \texttt{pwd} 显示绝对路径，\texttt{ls -la} 列出 input 下的文件。
...
\textbf{步骤3：复制文件保留目录结构。} 使用 \texttt{find . -name "*.txt" -print0 | tar -cf - --null -T -} 将文件打包，再通过管道在目标目录 \texttt{tar -xvf -} 解包，正确处理含空格的文件名。
...
\textbf{步骤4：设置权限与生成 inventory。} 用 \texttt{find -type d -exec chmod 750} 和 \texttt{find -type f -exec chmod 640} 分别设置目录和文件权限，用 \texttt{find . -type f -printf "\%P \%s\textbackslash n"} 生成清单。
...
\begin{enumerate}
    \item \textbf{含空格文件名处理}：\texttt{find -print0} 配合 \texttt{tar --null -T -} 通过管道传输文件名，\texttt{-print0} 以空字符分隔，避免空格和特殊字符导致的分词问题。
    \item \texttt{tar} 管道复制：\texttt{find ... | tar -cf - --null -T - | (cd 目标目录 \&\& tar -xvf -)} 在子 shell 中切换目录解包，保留相对目录结构。
    \item \textbf{权限数字表示}：750 = rwxr-x---（属主全权限、属组读执行、其他无权限）；640 = rw-r-----（属主读写、属组只读、其他无权限）。
    \item \texttt{find -printf "\%P \%s\textbackslash n"}：\texttt{\%P} 输出相对路径、\texttt{\%s} 输出字节数，一条命令即可生成机器可读清单。
\end{enumerate}
...
本次课后练习来自课程参考资料 \href{https://mlbo.github.io/missing-semester-cn-2026/2026/course-shell/}{MIT Missing Semester 2026 第1讲：课程概览与 Shell} 和 \href{https://mlbo.github.io/missing-semester-cn-2026/2026/version-control/}{第5讲：版本控制与 Git} 的课后习题，共完成 17 道题（Shell 9 道 + Git 8 道），涵盖目录操作、man 手册、文件创建、脚本编写、权限管理、管道重定向、sysfs 系统信息、Git 历史检索、stash 与别名等主题。
...
\subsection{Git 练习：克隆仓库与历史检索（10--17）}

\begin{enumerate}
    \setcounter{enumi}{9}
    \item \textbf{克隆课程仓库}：\texttt{git clone https://github.com/missing-semester/missing-semester.git}，用 \texttt{--depth 50} 做浅克隆，减少下载量。
    \item \textbf{可视化版本历史}：\texttt{git log --all --graph --decorate --oneline} 一次性展示所有分支的合并结构。
    \item \textbf{谁最后修改了 README.md}：\texttt{git log -1 --format="\%h \%an \%s" -- README.md}，结果为 commit 49f676c，作者 Anish Athalye。
    \item \textbf{查找引入某行的提交}：\texttt{git log -S "collections:" -- \_config.yml} 用 pickaxe 搜索字符串出现/消失的提交，定位到 e9ba75d。
    \item \textbf{配置 git graph 别名}：在 \texttt{\textasciitilde/.gitconfig} 中加入 \texttt{[alias] graph = log --all --graph --decorate --oneline}，之后 \texttt{git graph} 一条命令即可看历史图。
    \item \textbf{git stash 演示}：修改 README 后 \texttt{git stash}，工作区恢复干净；\texttt{git stash pop} 把改动恢复回来。stash 适合"切分支前临时藏一下未完成工作"的场景。
    \item \textbf{全局 .gitignore}：在 \texttt{\textasciitilde/.gitconfig} 配置 \texttt{core.excludesfile} 指向全局忽略文件，把 \texttt{.DS\_Store}、\texttt{.venv/} 等通用临时文件全局忽略。
    \item \texttt{git clone --depth 1} 浅克隆：只拉最新一次提交，适合只看代码不需要历史的场景。
\end{enumerate}
