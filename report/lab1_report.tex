...
\author{丁同学 \quad 学号：2023001234\\[0.3cm]
计算机科学与技术学院}
\date{2026年8月30日}
...
课程要求每次课完成超过 10 个课后练习实例。本次课后练习围绕 Shell 命令行基础展开，共完成 10 个实例，涵盖文件浏览、权限管理、管道重定向、文本搜索、列处理、文件查找、进程管理、压缩解压、符号链接和环境变量等主题。

\subsection{练习 1--5：文件与文本处理基础}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{practice_1_5.png}
\caption{课后练习 1--5：ls、chmod、管道、grep、awk}
\label{fig:practice1}
\end{figure}

\begin{enumerate}
    \item \textbf{ls 常用选项}：\texttt{ls -la} 显示长格式含隐藏文件，\texttt{ls -t} 按修改时间排序。
    \item \textbf{chmod 权限修改}：数字法 \texttt{chmod 755} 和符号法 \texttt{chmod u+x,g+w} 两种方式。
    \item \textbf{管道与重定向}：\texttt{sort | uniq -c} 统计词频，\texttt{> file} 将输出重定向到文件。
    \item \textbf{grep 文本搜索}：\texttt{grep pattern} 搜索匹配行，\texttt{-c} 计数，\texttt{-v} 排除匹配。
    \item \textbf{awk 列处理}：\texttt{\$1, \$2} 引用列，END 块计算平均值，条件过滤行。
\end{enumerate}

\subsection{练习 6--10：系统与高级操作}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{practice_6_10.png}
\caption{课后练习 6--10：find、ps、tar、软链接、环境变量}
\label{fig:practice2}
\end{figure}

\begin{enumerate}
    \setcounter{enumi}{5}
    \item \textbf{find 文件查找}：\texttt{-name} 按名称，\texttt{-type f} 限定文件，\texttt{-size} 按大小过滤。
    \item \textbf{ps 进程管理}：\texttt{ps aux} 显示所有进程详细信息，\texttt{ps -u user} 按用户过滤。
    \item \textbf{tar 压缩解压}：\texttt{tar czf} 创建 gzip 压缩包，\texttt{tar tzf} 查看压缩包内容。
    \item \textbf{软链接}：\texttt{ln -s target link} 创建符号链接，\texttt{ls -li} 显示 inode 号和链接指向。
    \item \textbf{环境变量与别名}：\texttt{export VAR=value} 设置环境变量，\texttt{alias} 创建命令别名。
\end{enumerate}
...
通过本次实验，我对系统开发工具有了更深入的理解，主要收获如下：

\subsection{Shell 编程的健壮性思维}

第1题中含空格文件名的处理让我深刻认识到 Shell 脚本中"引用"的重要性。在 Shell 中，未加引号的变量会经历单词拆分（word splitting）和通配符展开（globbing），这在处理含空格或特殊字符的文件名时会导致难以调试的错误。使用 \texttt{find -print0 | while IFS= read -r -d ''} 模式是处理任意文件名的"黄金法则"。

第2题的日志统计让我体会到 Shell 管道的强大：将简单工具（awk、sort、head）通过管道组合，可以完成复杂的数据处理任务。同时，\texttt{set -euo pipefail} 和正确的 stderr/退出码处理是编写"可用于生产"脚本的基本素养。

\subsection{Git 分支与冲突的本质}

第3题主动制造合并冲突的过程非常有价值。以前遇到冲突总是感到恐慌，通过亲手制造和解决冲突，我理解了冲突的本质：Git 的三方合并机制在两个分支修改了同一位置且无法自动调和时，将决定权交给开发者。冲突标记（\texttt{<<<<<<<}、\texttt{=======}、\texttt{>>>>>>>}）不是错误，而是 Git 提供的"决策辅助界面"。

\subsection{LaTeX 的编译机制与交叉引用}

第4题让我理解了 LaTeX"多次编译"的设计哲学。与所见即所得的文字处理器不同，LaTeX 的交叉引用、目录、参考文献都依赖辅助文件（.aux、.toc、.bbl），需要多次编译才能正确解析。这种"编译-链接"模式与 C/C++ 的构建过程异曲同工，体现了工具链设计的一致性。

\subsection{工具链思维的建立}

整个实验最核心的收获是"工具链思维"：从 Shell 命令行到 Git 版本控制，再到 LaTeX 文档构建，这些工具不是孤立的，而是构成了一个完整的开发工作流。掌握这些工具的目的不是记住命令，而是理解它们如何协同工作，让开发过程从"能运行"走向"可复现、可协作、可持续"。
...
提交哈希 & 提交信息 & 说明 \\
\midrule
(初始) & Initial commit & 仓库初始化 \\
(第1次) & Add q01: shell file operations with spaced filenames & 第1题代码 \\
(第2次) & Add q02: access log analysis script & 第2题代码 \\
(第3次) & Add q03: git merge conflict demonstration & 第3题仓库 \\
(第4次) & Add q04: latex technical report & 第4题文档 \\
(第5次) & Add practice: 10 shell exercises & 课后练习 \\
(第6次) & Add lab report with screenshots & 实验报告 \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{说明}：由于实验环境无法访问外部 GitHub/Gitee 服务，仓库在本地初始化并完成多次提交。实际提交时请将远程仓库地址配置为个人 GitHub 或 Gitee 账号后执行 \texttt{git push}。
